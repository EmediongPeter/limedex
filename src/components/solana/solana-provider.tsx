...
const WalletButtonBase = ({ compact, className, ...props }: WalletButtonProps) => {
  ...
  return (
    <div className="fixed bottom-4 left-4 right-4 bg-blue-50 dark:bg-blue-900/80 p-4 rounded-lg shadow-lg z-40 border border-blue-200 dark:border-blue-800 animate-fadeIn">
      <div className="flex justify-between items-start">
        <h3 className="font-medium text-blue-900 dark:text-blue-100">Mobile Wallet Connection</h3>
        <button 
      ...
      <p className="text-sm mt-2 text-blue-800 dark:text-blue-200">For the best experience, connect using the Phantom or Solflare mobile app.</p>
...
export function SolanaProvider({ children }: { children: ReactNode }) {
  ...
  // Show mobile guide after a short delay
  useEffect(() => {
    if (isMobile) {
      const timer = setTimeout(() => setShowMobileGuide(true), 1000);
  ...
  return (
    <ConnectionProvider endpoint={endpoint}>
      <WalletProvider wallets={wallets} onError={onError} autoConnect={!isMobile}>
        <WalletModalProvider>
          {isMobile && showMobileGuide && <MobileWalletGuide />}
          {children}
        </WalletModalProvider>
      </WalletProvider>
    </ConnectionProvider>
